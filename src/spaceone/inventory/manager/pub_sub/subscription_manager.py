...
from spaceone.inventory.model.pub_sub.subscription.cloud_service import (
    SubscriptionResource,
    SubscriptionResponse,
)
from spaceone.inventory.model.pub_sub.subscription.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class SubscriptionManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        subscription_conn: SubscriptionConnector = self.locator.get_connector(
        ...
        for subscription in subscriptions:
            try:
                ...
                subscription_display = {
                    "delivery_type": self._make_delivery_type(
                ...
                collected_cloud_services.append(
                    SubscriptionResponse({"resource": subscription_resource})
                )

            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "PubSub", "Subscription", subscription_id
                )
                error_responses.append(error_response)

        _LOGGER.debug(
            f"** PubSub Subscription Finished {time.time() - start_time} Seconds **"
        )
