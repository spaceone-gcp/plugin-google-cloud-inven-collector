...
from spaceone.inventory.model.pub_sub.schema.cloud_service import (
    SchemaResource,
    SchemaResponse,
)
from spaceone.inventory.model.pub_sub.schema.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class SchemaManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        schema_conn: SchemaConnector = self.locator.get_connector(
        ...
        for schema_name in schema_names:
            try:
                ...
                collected_cloud_services.append(
                    SchemaResponse({"resource": schema_resource})
                )
                time.sleep(0.1)
                ...
                error_response = self.generate_resource_error_response(
                    e, "PubSub", "Schema", schema_id
                )
                error_responses.append(error_response)

        _LOGGER.debug(
            f"** PubSub Scheam Finished {time.time() - start_time} Seconds **"
        )
