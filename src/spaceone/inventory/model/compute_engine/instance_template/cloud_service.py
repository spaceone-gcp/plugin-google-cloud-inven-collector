...
from spaceone.inventory.model.compute_engine.instance_template.data import (
    InstanceTemplate,
)
...
meta_available_policy = ItemDynamicLayout.set_fields(
    "Available policies",
    fields=[
        EnumDyField.data_source(
            "Preemptibility",
            "data.scheduling.preemptibility",
            default_badge={"primary": ["On"], "coral.600": ["Off"]},
        ),
        EnumDyField.data_source(
            "Automatic Restart",
            "data.scheduling.automatic_restart",
            default_badge={"primary": ["On"], "coral.600": ["Off"]},
        ),
        EnumDyField.data_source(
            "Host Maintenance",
...
# TAB - Service Account
it_meta_service_account = ItemDynamicLayout.set_fields(
    "Service Account",
    root_path="data.service_account",
    fields=[
        TextDyField.data_source("E-mail", "email"),
        ListDyField.data_source(
            "Scopes", "scopes", default_badge={"type": "outline", "delimiter": "<br>"}
        ),
    ],
)

instance_template = ListDynamicLayout.set_layouts(
    "Instance Template",
    layouts=[instance_template_meta, meta_available_policy, it_meta_service_account],
)

# TAB - Network
# instance_template_meta_network
it_meta_network = TableDynamicLayout.set_fields(
    "Network Interface",
...
meta_it_disk = TableDynamicLayout.set_fields(
    "Disks",
    ...
    fields=[
        ...
        EnumDyField.data_source(
            "Mode",
            "device_mode",
            default_badge={"indigo.500": ["READ_WRITE"], "coral.600": ["READ_ONLY"]},
        ),
        EnumDyField.data_source(
            "Auto Delete",
            "tags.auto_delete",
