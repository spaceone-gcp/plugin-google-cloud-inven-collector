...
class AppEngineInstanceV1Connector(GoogleCloudConnector):
    ...
    def list_instances(self, service_id, version_id, **query):
        """
        App Engine 인스턴스 목록을 조회합니다 (v1beta API).
        """
        instance_list = []
        query.update(
            {
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to list App Engine instances for version {version_id} (v1beta): {e}"
            )
    ...
    def get_instance(self, service_id, version_id, instance_id, **query):
        """
        특정 App Engine 인스턴스 정보를 조회합니다 (v1beta API).
        """
        try:
            request = (
                self.client.apps()
            ...
            _LOGGER.error(
                f"Failed to get App Engine instance {instance_id} (v1beta): {e}"
            )
            return None

    def list_all_instances(self, **query):
        """
        모든 App Engine 인스턴스를 조회합니다 (v1beta API).
        """
        all_instances = []
        ...
        try:
            ...
            services_response = (
                self.client.apps().services().list(appsId=self.project_id).execute()
            )
            ...
            for service in services:
                ...
                if service_id:
                    ...
                    versions_response = (
                        ...
                        .services()
                        .versions()
                        .list(appsId=self.project_id, servicesId=service_id)
                        .execute()
                    )
            ...
            _LOGGER.error(f"Failed to list all App Engine instances (v1beta): {e}")

        return all_instances

    def get_instance_metrics(self, service_id, version_id, instance_id, **query):
        """
        App Engine 인스턴스 메트릭을 조회합니다 (v1beta API).
        """
        try:
            ...
            memory_mb = (
                round(float(memory_bytes) / (1024 * 1024), 1) if memory_bytes else 0.0
            )

            metrics = {
                "memory_usage": memory_mb,  # MB 단위로 변환된 값
                "cpu_usage": instance_info.get("cpuUsage", 0),
                "request_count": instance_info.get("requests", 0),
            ...
            _LOGGER.info(
                f"Retrieved instance metrics for {instance_id}: status={metrics['vm_status']}"
            )
            return metrics
        except Exception as e:
            _LOGGER.error(
                f"Failed to get App Engine instance metrics for {instance_id} (v1beta): {e}"
            )
            return None

    def list_instances_by_status(self, service_id, version_id, status, **query):
        """
        특정 상태의 App Engine 인스턴스 목록을 조회합니다 (v1beta API).
        """
        try:
            ...
            _LOGGER.error(
                f"Failed to list App Engine instances by status {status} (v1beta): {e}"
            )
            return []

    def get_instance_details(self, service_id, version_id, instance_id, **query):
        """
        App Engine 인스턴스 상세 정보를 조회합니다 (v1beta API).
        """
        try:
            instance_info = self.get_instance(service_id, version_id, instance_id)
            if not instance_info:
            ...
            _LOGGER.error(
                f"Failed to get App Engine instance details for {instance_id} (v1beta): {e}"
            )
            return None
