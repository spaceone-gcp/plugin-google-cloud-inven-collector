...
from spaceone.inventory.model.pub_sub.topic.cloud_service import (
    TopicResource,
    TopicResponse,
)
...
class TopicManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        topic_conn: TopicConnector = self.locator.get_connector(
        ...
        for topic in topics:
            try:
                ...
                collected_cloud_services.append(
                    TopicResponse({"resource": topic_resource})
                )
            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "PubSub", "Topic", topic_id
                )
                error_responses.append(error_response)

        _LOGGER.debug(f"** PubSub Topic Finished {time.time() - start_time} Seconds **")
