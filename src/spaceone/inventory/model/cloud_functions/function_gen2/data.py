...
from schematics.types import ModelType, ListType, StringType, DictType, FloatType
...
class FunctionGen2(BaseResource):
    ...
    environment = StringType(
        choices=("ENVIRONMENT_UNSPECIFIED", "GEN_1", "GEN_2"), serialize_when_none=False
    )
    description = StringType(serialize_when_none=False)
    build_config = ModelType(
        BuildConfig, serialize_when_none=False, deserialize_from="buildConfig"
    )
    service_config = ModelType(
        ServiceConfig, serialize_when_none=False, deserialize_from="serviceConfig"
    )
    ...
    state_messages = ListType(
        ModelType(StateMessage),
        serialize_when_none=False,
        deserialize_from="stateMessages",
    )
