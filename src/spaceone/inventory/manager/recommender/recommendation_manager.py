...
class RecommendationManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        recommendation_conn: RecommendationConnector = self.locator.get_connector(
            RecommendationConnector, **params
        )
            ...
            for recommendation in recommendations:
                try:
                    ...
                    display = {
                        ...
                        "recommender_id_name": self.recommender_map[recommender_id][
                            "name"
                        ],
                        "recommender_id_description": self.recommender_map[
                            recommender_id
                        ]["short_description"],
                        "priority_display": self.convert_readable_priority(
                        ...
                        "operation_actions": self._get_actions(
                    ...
                    if cost_info := recommendation["primaryImpact"].get(
                        "costProjection"
                    ):
                        cost = cost_info.get("cost", {})
                        (
                            display["cost"],
                            display["cost_description"],
                        ) = self._change_cost_to_description(cost)

                    if insights := recommendation["associatedInsights"]:
                        insight_conn: InsightConnector = self.locator.get_connector(
                            InsightConnector, **params
                        )
                    ...
                    preprocessed_recommendations.append(
                        recommendation_data.to_primitive()
                    )

                except Exception as e:
                    _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                    error_response = self.generate_resource_error_response(
                        e, "Recommender", "Recommendation", recommendation
                    )
        ...
        for recommender in merged_recommenders:
            try:
                ...
                (
                    recommender["state"],
                    recommender["primary_priority_level"],
                ...
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "Recommender", "Recommendation", recommender
                )
                error_responses.append(error_response)

        _LOGGER.debug(
            f"** Recommender Recommendation Finished {time.time() - start_time} Seconds **"
        )
            ...
            changed_insights.append(
                {
                    ...
                    "etag": insight["etag"],
                    "target_resources": self._change_target_resources(
    ...
    def _change_target_resources(self, resources):
        ...
        for resource in resources:
            new_target_resources.append(
                {"name": resource, "display_name": self._change_resource_name(resource)}
            )
