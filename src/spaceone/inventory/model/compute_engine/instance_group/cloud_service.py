...
instance_group_item = ItemDynamicLayout.set_fields(
    ...
    fields=[
        ...
        TextDyField.data_source(
            "Min Replicas", "data.autoscaler.autoscaling_policy.min_num_replicas"
        ),
        TextDyField.data_source(
            "Max Replicas", "data.autoscaler.autoscaling_policy.max_num_replicas"
        ),
...
autoscaling_policy = ItemDynamicLayout.set_fields(
    ...
    fields=[
        EnumDyField.data_source(
            "Mode",
            "data.autoscaler.autoscaling_policy.mode",
            default_badge={"indigo.500": ["ON"], "coral.600": ["OFF"]},
        ),
        TextDyField.data_source(
            "Minimum Replica Counts",
            "data.autoscaler.autoscaling_policy.min_num_replicas",
        ),
        TextDyField.data_source(
            "Maximum Replica Counts",
            "data.autoscaler.autoscaling_policy.max_num_replicas",
        ),
        TextDyField.data_source(
            "Cool Down Period (Seconds)",
            "data.autoscaler.autoscaling_policy.cool_down_period_sec",
        ),
        TextDyField.data_source(
            "CPU Utilization",
...
autoscaling_layout = ListDynamicLayout.set_layouts(
    "Auto Scaler", layouts=[autoscaler, autoscaling_policy]
)

autohealing_layout = TableDynamicLayout.set_fields(
    "AutoHealing",
    root_path="data.instance_group_manager.auto_healing_policies",
    fields=[
...
instance_layout = TableDynamicLayout.set_fields(
    "Instance",
...
instance_group_meta = CloudServiceMeta.set_layouts(
    [instance_group_layout, instance_layout, autoscaling_layout, autohealing_layout]
)
...
class InstanceGroupResource(ComputeEngineResource):
    ...
    _metadata = ModelType(
        CloudServiceMeta, default=instance_group_meta, serialized_name="metadata"
    )
