...
VM Instance
"""
vm_instance = ItemDynamicLayout.set_fields(
    "VM Instance",
    fields=[
        ...
        EnumDyField.data_source(
            "Preemptible",
            "data.google_cloud.scheduling.preemptible",
            default_badge={"indigo.500": ["true"], "coral.600": ["false"]},
        ),
        EnumDyField.data_source(
            "Is Managed Instance in Instance Group",
            "data.google_cloud.is_managed_instance",
            default_badge={"indigo.500": ["true"], "coral.600": ["false"]},
        ),
        TextDyField.data_source("Instance Type", "data.compute.instance_type"),
        EnumDyField.data_source(
            "Has GPU",
            "data.display.has_gpu",
            default_badge={"indigo.500": ["True"], "coral.600": ["False"]},
        ),
        ...
        TextDyField.data_source(
            "Reservation Affinity", "data.google_cloud.reservation_affinity"
        ),
        TextDyField.data_source("Self link", "data.google_cloud.self_link"),
        EnumDyField.data_source(
            "Deletion Protection",
            "data.google_cloud.deletion_protection",
            default_badge={"indigo.500": ["true"], "coral.600": ["false"]},
        ),
...
google_cloud_vpc = ItemDynamicLayout.set_fields(
    "VPC",
    fields=[
...
instance_group_manager = ItemDynamicLayout.set_fields(
    "InstanceGroupManager",
    fields=[
        TextDyField.data_source("Auto Scaler", "data.autoscaler.name"),
        TextDyField.data_source("Auto Scaler ID", "data.autoscaler.id"),
        TextDyField.data_source(
            "Instance Group Name", "data.autoscaler.instance_group.name"
        ),
        TextDyField.data_source(
            "Instance Template Name",
            "data.autoscaler.instance_group.instance_template_name",
        ),
    ],
)

operating_system_manager = ItemDynamicLayout.set_fields(
    "Operating System",
    fields=[
...
hardware_manager = ItemDynamicLayout.set_fields(
    "Hardware",
...
ssh_keys = SimpleTableDynamicLayout.set_tags(
    "SSH Keys",
    root_path="data.google_cloud.ssh_keys.ssh_keys",
    fields=[
...
ssh_options = ItemDynamicLayout.set_fields(
    "SSH Key Options",
    root_path="data.google_cloud.ssh_keys",
    fields=[
        TextDyField.data_source("Block project-wide SSH keys", "block_project_ssh_keys")
    ],
)

service_accounts = TableDynamicLayout.set_fields(
...
compute_engine = ListDynamicLayout.set_layouts(
    "Compute Engine",
    layouts=[
        vm_instance,
        google_cloud_vpc,
...
firewall = TableDynamicLayout.set_fields(
    "Firewalls",
    ...
    fields=[
        ...
        EnumDyField.data_source(
            "Direction",
            "direction",
            default_badge={"indigo.500": ["ingress"], "coral.600": ["egress"]},
        ),
        EnumDyField.data_source(
            "Action",
            "action",
...
lb = TableDynamicLayout.set_fields(
    ...
    fields=[
        ...
        EnumDyField.data_source(
            "Scheme",
            "scheme",
            default_badge={"indigo.500": ["EXTERNAL"], "coral.600": ["INTERNAL"]},
        ),
    ],
)
...
labels = TableDynamicLayout.set_fields(
    "Labels",
    root_path="data.google_cloud.labels",
    fields=[
...
tags = TableDynamicLayout.set_fields(
    "Tags",
    root_path="data.google_cloud.tags",
    fields=[TextDyField.data_source("Item", "key")],
)
...
class VMInstanceResource(ComputeEngineResource):
    ...
    _metadata = ModelType(
        CloudServiceMeta, default=vm_instance_meta, serialized_name="metadata"
    )
