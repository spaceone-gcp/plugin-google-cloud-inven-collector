...
from spaceone.inventory.model.compute_engine.instance_group.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
from spaceone.inventory.model.compute_engine.instance_group.cloud_service import (
    InstanceGroupResource,
    InstanceGroupResponse,
)
from spaceone.inventory.model.compute_engine.instance_group.data import (
...
class InstanceGroupManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        instance_group_conn: InstanceGroupConnector = self.locator.get_connector(
        ...
        for instance_group in instance_groups:
            try:
                    ...
                    instance_group.update(
                        {
                            ...
                            "instance_group_manager": InstanceGroupManagers(
                                match_instance_group_manager, strict=False
                            ),
                        }
                    )
                        ...
                        instance_group.update(
                            {
                                "template": InstanceTemplate(
                                    match_instance_template, strict=False
                                )
                            }
                        )
                ...
                collected_cloud_services.append(
                    InstanceGroupResponse({"resource": instance_group_resource})
                )
            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "ComputeEngine", "InstanceGroup", instance_group_id
                )
                error_responses.append(error_response)

        _LOGGER.debug(
            f"** Instance Group Finished {time.time() - start_time} Seconds **"
        )
