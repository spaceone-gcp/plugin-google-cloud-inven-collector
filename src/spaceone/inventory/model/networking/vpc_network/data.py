...
class Peering(Model):
    ...
    network = StringType()
    state = StringType()
    state_details = StringType(
        deserialize_from="stateDetails", serialize_when_none=False
    )
    auto_create_routes = BooleanType(
        deserialize_from="autoCreateRoutes", serialize_when_none=False
    )
    export_custom_routes = BooleanType(
        deserialize_from="exportCustomRoutes", serialize_when_none=False
    )
    import_custom_routes = BooleanType(
        deserialize_from="importCustomRoutes", serialize_when_none=False
    )
    exchange_subnet_routes = BooleanType(
        deserialize_from="exchangeSubnetRoutes", serialize_when_none=False
    )
    export_subnet_routes_with_public_ip = BooleanType(
        deserialize_from="exportSubnetRoutesWithPublicIp", serialize_when_none=False
    )
    import_subnet_routes_with_public_ip = BooleanType(
        deserialize_from="importSubnetRoutesWithPublicIp", serialize_when_none=False
    )
...
class Route(Model):
    ...
    nextHopInstance = StringType(
        deserialize_from="nextHopInstance", serialize_when_none=False
    )
    nextHopIp = StringType(deserialize_from="nextHopIp", serialize_when_none=False)
    nextHopNetwork = StringType(
        deserialize_from="nextHopNetwork", serialize_when_none=False
    )
    nextHopGateway = StringType(
        deserialize_from="nextHopGateway", serialize_when_none=False
    )
    nextHopPeering = StringType(
        deserialize_from="nextHopPeering", serialize_when_none=False
    )
...
class VPNRoutingConfig(Model):
    routing_mode = StringType(
        choices=("REGIONAL", "GLOBAL"), deserialize_from="routingMode"
    )
...
class Firewall(Model):
    ...
    source_ranges = ListType(
        StringType(), deserialize_from="sourceRanges", serialize_when_none=False
    )
    destination_ranges = ListType(
        StringType(), deserialize_from="destinationRanges", serialize_when_none=False
    )
    source_tags = ListType(
        StringType(), deserialize_from="sourceTags", serialize_when_none=False
    )
    target_tags = ListType(
        StringType(), deserialize_from="targetTags", serialize_when_none=False
    )
    ...
    log_config = ModelType(
        LogConfig, deserialize_from="logConfig", serialize_when_none=False
    )
    self_link = StringType(deserialize_from="selfLink")
    creation_timestamp = DateTimeType(deserialize_from="creationTimestamp")
