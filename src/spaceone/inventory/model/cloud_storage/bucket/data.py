...
class BucketAccessControls(Model):
    ...
    project_team = ModelType(
        ProjectTeam, deserialize_from="projectTeam", serialize_when_none=False
    )
...
class Action(Model):
    ...
    storage_class = StringType(
        deserialize_from="storageClass", serialize_when_none=False
    )


class Condition(Model):
    age = IntType(deserialize_from="age", serialize_when_none=False)
    created_before = StringType(
        deserialize_from="createdBefore", serialize_when_none=False
    )
    custom_time_before = StringType(
        deserialize_from="customTimeBefore", serialize_when_none=False
    )
    days_since_custom_time = IntType(
        deserialize_from="daysSinceCustomTime", serialize_when_none=False
    )
    days_since_non_current_time = IntType(
        deserialize_from="daysSinceNoncurrentTime", serialize_when_none=False
    )
    is_live = BooleanType(deserialize_from="isLive", serialize_when_none=False)
    matches_storage_class = ListType(
        StringType(), deserialize_from="matchesStorageClass", serialize_when_none=False
    )
    non_current_time_before = StringType(
        deserialize_from="noncurrentTimeBefore", serialize_when_none=False
    )
    num_newer_versions = IntType(
        deserialize_from="numNewerVersions", serialize_when_none=False
    )
...
class IAMConfiguration(Model):
    bucket_policy_only = ModelType(
        BucketAccess, deserialize_from="bucketPolicyOnly", serialize_when_none=False
    )
    uniform_bucket_level_access = ModelType(
        BucketAccess,
        deserialize_from="uniformBucketLevelAccess",
...
class Storage(BaseResource):
    ...
    default_object_acl = ListType(
        ModelType(ObjectACL),
        default=[],
        serialize_when_none=False,
        deserialize_from="defaultObjectAcl",
    )
    public_access = StringType(
        choices=("Subject to object ACLs", "Not Public", "Public to Internet")
    )
    ...
    default_storage_class = StringType(
        choices=("Standard", "Nearline", "Coldline", "Archive")
    )
    iam_configuration = ModelType(
        IAMConfiguration, deserialize_from="iamConfiguration", serialize_when_none=False
    )
