...
sql_meta_instance = ItemDynamicLayout.set_fields(
    ...
    fields=[
        ...
        TextDyField.data_source(
            "Service Account", "data.service_account_email_address"
        ),
    ],
)
...
# TAB - Connections
sql_meta_connection = ItemDynamicLayout.set_fields(
    "Connection",
    fields=[
        TextDyField.data_source("Connection name", "data.connection_name"),
        ListDyField.data_source(
            "IP Address Type",
            "data.ip_addresses",
            default_badge={"type": "outline", "sub_key": "type", "delimiter": "<br>"},
        ),
        ListDyField.data_source(
            "Public IP Address",
            "data.ip_addresses",
        ...
        DateTimeDyField.data_source(
            "SSL certificates Creation Time", "data.server_ca_cert.create_time"
        ),
        DateTimeDyField.data_source(
            "SSL certificates Expiration Time", "data.server_ca_cert.expiration_time"
        ),
    ],
)
...
# TAB - User
sql_meta_user = TableDynamicLayout.set_fields(
    "User",
    "data.users",
    fields=[
...
# TAB - Backup
sql_meta_backup = ItemDynamicLayout.set_fields(
    "Backup",
    fields=[
        EnumDyField.data_source(
            "Enabled",
            "data.settings.backup_configuration.enabled",
            default_badge={"indigo.500": ["true"], "coral.600": ["false"]},
        ),
        TextDyField.data_source(
            "Location", "data.settings.backup_configuration.location"
        ),
        TextDyField.data_source(
            "Start Time", "data.settings.backup_configuration.start_time"
        ),
        EnumDyField.data_source(
            "Binary Log Enabled",
...
class InstanceResource(CloudSQLResource):
    ...
    _metadata = ModelType(
        CloudServiceMeta, default=cloud_sql_meta, serialized_name="metadata"
    )
