...
from spaceone.inventory.model.compute_engine.instance_template.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
from spaceone.inventory.model.compute_engine.instance_template.cloud_service import (
        ...
        for inst_template in instance_templates:
            try:
                ...
                collected_cloud_services.append(
                    InstanceTemplateResponse({"resource": instance_template_resource})
                )
            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "ComputeEngine", "InstanceTemplate", inst_template_id
                )
