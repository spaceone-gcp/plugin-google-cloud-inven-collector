...
class VPCGatewayConnector(GoogleCloudConnector):
    ...
    def list_nat_gateways(self, **query):
        ...
        try:
            _LOGGER.debug(
                f"Listing routers for NAT gateways in project {self.project_id}"
            )
            request = self.client.routers().aggregatedList(**query)
            while request is not None:
                response = request.execute()
                for region_key, routers_scoped_list in response.get(
                    "items", {}
                ).items():
                    if "routers" in routers_scoped_list:
                        ...
                        _LOGGER.debug(
                            f"Found {len(routers_scoped_list['routers'])} routers in {region_name}"
                        )
                        ...
                        for router in routers_scoped_list["routers"]:
                            ...
                            if "nats" in router:
                                _LOGGER.debug(
                                    f"Router {router.get('name')} has {len(router['nats'])} NAT configurations"
                                )
                                for nat in router["nats"]:
                                    nat_gateway = {
                                        ...
                                        "self_link": router.get(
                                            "selfLink"
                                        ),  # NAT Gateway는 Router의 일부이므로 Router의 selfLink 사용
                                        "creation_timestamp": router.get(
                                            "creationTimestamp"
                                        ),
                                        "description": router.get("description", ""),
                                        "network": router.get("network", ""),
                                        "nat_ip_allocate_option": nat.get(
                                            "natIpAllocateOption"
                                        ),
                                        ...
                                        "enable_endpoint_independent_mapping": nat.get(
                                            "enableEndpointIndependentMapping"
                                        ),
                                        "icmp_idle_timeout_sec": nat.get(
                                            "icmpIdleTimeoutSec"
                                        ),
                                        "tcp_established_idle_timeout_sec": nat.get(
                                            "tcpEstablishedIdleTimeoutSec"
                                        ),
                                        "tcp_transitory_idle_timeout_sec": nat.get(
                                            "tcpTransitoryIdleTimeoutSec"
                                        ),
                                        "tcp_time_wait_timeout_sec": nat.get(
                                            "tcpTimeWaitTimeoutSec"
                                        ),
                                        "udp_idle_timeout_sec": nat.get(
                                            "udpIdleTimeoutSec"
                                        ),
    ...
    def list_vpn_gateways(self, **query):
        ...
        try:
            ...
            while request is not None:
                response = request.execute()
                for region_key, vpn_gateways_scoped_list in response.get(
                    "items", {}
                ).items():
            ...
            while request is not None:
                response = request.execute()
                for region_key, target_vpn_gateways_scoped_list in response.get(
                    "items", {}
                ).items():
                    if "targetVpnGateways" in target_vpn_gateways_scoped_list:
                        region_name = self._extract_region_from_key(region_key)

                        for target_vpn_gateway in target_vpn_gateways_scoped_list[
                            "targetVpnGateways"
                        ]:
