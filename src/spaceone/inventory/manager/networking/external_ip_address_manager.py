...
from spaceone.inventory.model.networking.external_ip_address.cloud_service import *
from spaceone.inventory.model.networking.external_ip_address.data import (
    ExternalIpAddress,
)
from spaceone.inventory.connector.networking.external_ip_address import (
    ExternalIPAddressConnector,
)
from spaceone.inventory.model.networking.external_ip_address.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class ExternalIPAddressManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        exp_conn: ExternalIPAddressConnector = self.locator.get_connector(
        ...
        for external_ip_addr in all_external_ip_addresses:
            try:
                ...
                external_ip_addr.update(
                    {
                        ...
                        "status_display": external_ip_addr.get("status", "")
                        .replace("_", " ")
                        .title(),
                    }
                )
                if external_ip_addr.get("selfLink") is None:
                    external_ip_addr.update(
                        {
                            "self_link": self._get_external_self_link_when_its_empty(
                                external_ip_addr
                            )
                        }
                    )
                ...
                collected_cloud_services.append(
                    ExternalIpAddressResponse({"resource": external_ip_addr_resource})
                )
            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "VPC", "ExternalIPAddress", external_ip_addr_id
                )
                error_responses.append(error_response)

        _LOGGER.debug(
            f"** External IP Address Finished {time.time() - start_time} Seconds **"
        )
                ...
                ip_addr.update(
                    {
                        ...
                        "ip_version_display": self._get_ip_address_version(
                            ip_addr.get("address")
                        ),
                        "network_tier_display": ip_addr.get(
                            "networkTier", ""
                        ).capitalize(),
                        "is_ephemeral": "Static",
                    }
                ...
                forwarding_rule.update(
                    {
                        ...
                        "network_tier_display": forwarding_rule.get(
                            "networkTier", ""
                        ).capitalize(),
                ...
                for external_ip_info in external_ip_infos:
                    if (
                        ...
                        external_ip = {
                            ...
                            "network_tier_display": external_ip_info.get(
                                "networkTier", ""
                            ).capitalize(),
                            "status": "IN_USE",
                            "ip_version_display": self._get_ip_address_version(
