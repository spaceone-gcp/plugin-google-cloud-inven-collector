...
from spaceone.inventory.model.compute_engine.machine_image.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
from spaceone.inventory.model.compute_engine.machine_image.cloud_service import (
    MachineImageResource,
    MachineImageResponse,
)
from spaceone.inventory.model.compute_engine.machine_image.data import (
    MachineType,
    MachineImage,
    Disk,
)
...
class MachineImageManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        machine_image_conn: MachineImageConnector = self.locator.get_connector(
        ...
        for machine_image in machine_images:
            try:
                ...
                machine_image.update(
                    {
                        ...
                        "deletion_protection": properties.get(
                            "deletionProtection", False
                        ),
                        "machine_type": MachineType(
                            self.get_machine_type(properties), strict=False
                        ),
                        ...
                        "total_storage_display": self._convert_size(
                ...
                if len(svc_account) > 0:
                    machine_image.update(
                        {"service_account": self._get_service_account(svc_account)}
                    )
                ...
                collected_cloud_services.append(
                    MachineImageResponse({"resource": machine_image_resource})
                )
            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "ComputeEngine", "MachineImage", machine_image_id
                )
                error_responses.append(error_response)

        _LOGGER.debug(
            f"** Machine Image Finished {time.time() - start_time} Seconds **"
        )
    ...
    def get_network_interface(self, instance):
        ...
        for idx, network_interface in enumerate(instance.get("networkInterfaces", [])):
            ...
            network_interface_vo = {
                ...
                "network_tier_display": (
                    access_configs[0].get("networkTier")
                ...
                "subnetwork_display": self.get_param_in_url(
    ...
    def get_machine_type(self, instance):
        machine_vo = {"machine_type": instance.get("machineType", "")}
    ...
    def _get_scheduling(properties):
        ...
        return {
            ...
            "automatic_restart": (
                "On" if scheduling.get("automaticRestart", False) else "Off"
            ),
