...
from spaceone.inventory.model.cloud_functions.function_gen1.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
    cst_function,
)
from spaceone.inventory.model.cloud_functions.function_gen1.cloud_service import (
    FunctionResource,
    FunctionResponse,
)
...
class FunctionGen1Manager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        self.function_conn: FunctionGen1Connector = self.locator.get_connector(
        ...
        for function in functions:
            try:
                ...
                if http_trigger := function.get("httpsTrigger"):
                    display.update({"trigger": "HTTP", "http_url": http_trigger["url"]})

                if event_trigger := function.get("eventTrigger"):
                    trigger = self._get_display_name(event_trigger.get("service"))
                    display.update({"trigger": trigger, "event_provider": trigger})
        ...
        _LOGGER.debug(
            f"** Function Gen1 Finished {time.time() - start_time} Seconds **"
        )
