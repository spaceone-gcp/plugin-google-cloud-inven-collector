...
from spaceone.inventory.manager.compute_engine.vm_instance.disk_manager_resource_helper import (
...
from spaceone.inventory.model.compute_engine.instance.cloud_service import (
    VMInstanceResource,
    VMInstanceResponse,
)
...
class VMInstanceManager(GoogleCloudManager):
    ...
    def collect_cloud_service(
        ...
        for compute_vm in compute_vms:
            ...
            try:
                ...
                _LOGGER.error(
                    f"[list_resources] vm_id => {vm_id}, error => {e}", exc_info=True
                )
                error_response = self.generate_resource_error_response(
                    e, "ComputeEngine", "Instance", vm_id
                )
    ...
    def get_all_resources(self, project_id) -> dict:
        instancegroup_manager_helper: InstanceGroupManagerResourceHelper = (
    ...
    def get_vm_instance_resource(
        self, project_id, zone_info, instance, all_resources
    ) -> VMInstanceResource:
        ...
        loadbalancer_manager_helper: LoadBalancerManagerResourceHelper = (
            LoadBalancerManagerResourceHelper()
        )
        ...
        firewall_manager_helper: FirewallManagerResourceHelper = (
            FirewallManagerResourceHelper()
        )
        autoscaler_vo = auto_scaler_manager_helper.get_autoscaler_info(
            instance, instance_group, autoscaler
        )
        load_balancer_vos = loadbalancer_manager_helper.get_loadbalancer_info(
            instance,
            instance_group,
            backend_svcs,
            url_maps,
        ...
        server_data.update(
            {
                ...
                "instance_size": server_data.get("data", {})
                .get("hardware", {})
                .get("core", 0),
                "launched_at": server_data.get("data", {})
                .get("compute", {})
                .get("launched_at", ""),
                "tags": labels,
