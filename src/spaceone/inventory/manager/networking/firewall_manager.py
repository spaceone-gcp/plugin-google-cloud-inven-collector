...
class FirewallManager(GoogleCloudManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        firewall_conn: FirewallConnector = self.locator.get_connector(
        ...
        for firewall in firewalls:
            try:
                ...
                display = {
                    "enforcement": (
                        "Disabled" if firewall.get("disabled") else "Enabled"
                    ),
                ...
                firewall.update(
                    {
                        ...
                        "applicable_instance": self._get_matched_instance(
                ...
                firewall.update({
                    "google_cloud_monitoring": self.set_google_cloud_monitoring(
                        project_id, "firewall", firewall_id, [
                            {"key": "resource.labels.firewall_id", "value": firewall_id}
                        ]
                    ),
                ...
                collected_cloud_services.append(
                    FirewallResponse({"resource": firewall_resource})
                )
            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "VPC", "Firewall", firewall_id
                )
